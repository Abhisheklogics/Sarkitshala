...
export default async function Page({ params }) {
  ...
  return (
    ...
    <div className="h-fit w-full p-5 bg-white bg-cover rounded-xl leading-8 text-justify break-words 
      ...
      <h1 className="md:font-light font-bold text-2xl md:text-center md:text-2xl">{data.ExperimentName}</h1>
      ...
      <h2 className="md:text-2xl md:mt-10  text-2xl  md:font-light font-bold mt-10">Hardware components</h2>
      <p className="md:mt-4">{data.component}</p>

      <div className="md:ml-12 md:mt-8">
        <Image height={400} width={700} className="ml-1 md:ml-[-10px] w-fit h-fit mt-10" loading="lazy" src={data.image2} alt="Experiment Image" />
      </div>

      <h2 className="md:text-2xl md:mt-14  text-2xl md:font-light font-bold  mt-10">{data.pinDiagramInfo}</h2>

      {data.ExperimentId == '1' || data.ExperimentId == '2' || data.ExperimentId == '3' || data.ExperimentId == '4' || data.ExperimentId == '5' || data.ExperimentId == '7' ? (
        <div className="">
          <Image height={500} width={400} className="md:mt-20 md:ml-40 mt-10" loading="lazy" src={data.image3} alt="Experiment Image" />
        </div>
      ) : (
        <div className="md:ml-20 md:h-[300px]">
          <Image height={300} width={200} className="ml-40 md:mt-24" loading="lazy" src={data.image3} alt="Experiment Image" />
        </div>
      )}

      <h2 className="md:text-2xl md:font-light font-bold text-2xl  mt-12">{data.CircuitDiagramInfo}</h2>

      <div className="md:ml-32 md:mt-10">
        <Image height={400} width={500} className="md:ml-24    mt-10" loading="lazy" src={data.image4} alt="Experiment Image" />
      </div>

      <h1 className="md:text-2xl text-2xl md:font-light font-bold mt-10">Steps:</h1>
      ...
      <h2 className="md:mt-4 md:text-2xl text-2xl md:font-light font-bold mt-10">Code:</h2>
      <CodeBox language={'c'} num={data.ExperimentId} exNam='esp' />

      <h3 className="md:text-2xl text-2xl md:mt-6 mt-10 md:font-light font-bold">Result</h3>
