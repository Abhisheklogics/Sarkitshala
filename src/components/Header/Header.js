...
export default function Header() {
  ...
  const NavLink = ({ href, label, external = false }) => {
    ...
    const active = isActive
      ? "text-blue-400 after:absolute after:bottom-0 after:left-1/2 after:-translate-x-1/2 after:w-full after:h-[2px] after:bg-gradient-to-r after:from-blue-400 after:to-purple-500 after:rounded"
  ...
  return (
    <header className="fixed top-0 inset-x-0 z-50 backdrop-blur-lg bg-[#0b1120]/70 border-b border-white/10 shadow-md">
        ...
        <nav className=" hidden md:flex items-center  space-x-6">
          ...
          <div className="relative group">
            ...
            >
              IoT
              <svg
                className="ml-1 h-4 w-4 group-hover:rotate-180 transition-transform"
                fill="none"
                stroke="currentColor"
                strokeWidth={2}
                viewBox="0 0 24 24"
      ...
      >
        <ul className="space-y-4 px-6 py-4 border-t border-gray-700 bg-[#0b1120]/90 backdrop-blur-xl">
          {/* IoT Mobile Dropdown */}
          <li>
            <button
            ...
            <ul
              className={`mt-2 pl-4 border-l border-gray-600 text-sm space-y-3 transition-all ${
