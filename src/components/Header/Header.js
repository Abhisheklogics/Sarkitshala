...
export default function Header() {
  ...
  return (
   <header className="fixed top-0 inset-x-0 z-50 backdrop-blur-xl bg-[#0b1120]/80 border-b border-white/10 shadow-lg">
  <div className="mx-auto flex max-w-7xl items-center justify-between md:justify-center px-4 py-3 md:px-6 lg:px-8">
   
    <Link
      href="/"
      className="flex items-center space-x-4 text-2xl font-extrabold"
    >
      <span className="md:ml-[-230px] bg-gradient-to-r from-blue-400 via-indigo-400 to-purple-500 bg-clip-text text-transparent hover:from-purple-500 hover:to-blue-400 transition-all duration-500">
        Sarkitshala
      </span>
    </Link>
    ...
    <nav className="hidden md:flex items-center space-x-6">
      ...
      <div className="relative group">
        ...
        <ul className="absolute left-0 mt-3 w-56 rounded-xl bg-zinc-900/95 border border-gray-700 shadow-2xl opacity-0 scale-95 invisible group-hover:visible group-hover:opacity-100 group-hover:scale-100 transition-all duration-300 divide-y divide-gray-700 z-50">
          {iotLinks.map(link => (
            <li
              key={link.href}
              className="px-4 py-2 hover:bg-zinc-800/70 rounded-lg transition-colors"
            >
              <NavLink {...link} />
            </li>
          ))}
    ...
    <button
      ...
      className="flex h-10 w-10 items-center justify-center rounded-xl text-white hover:bg-white/10 md:hidden transition"
    >
      {mobileOpen ? (
        <svg
          className="h-6 w-6"
  ...
  >
    <ul className="space-y-4 px-6 py-4 border-t border-gray-700 bg-[#0b1120]/90 backdrop-blur-xl rounded-b-2xl shadow-lg">
      <li>
        <button
          onClick={() => setMobileIotOpen(!mobileIotOpen)}
          className="flex w-full items-center justify-between text-sm font-semibold text-gray-300 hover:text-blue-400 transition"
        >
          IOT
          <svg
        ...
        <ul
          className={`mt-2 pl-4 border-l border-gray-600 text-sm space-y-4 transition-all duration-500 ${
            mobileIotOpen ? "max-h-96" : "max-h-0 overflow-hidden"
          }`}
        >
          {iotLinks.map(link => (
            <li key={link.href}>
              <NavLink {...link} className="hover:text-purple-400 transition" />
            </li>
          ))}
        </ul>
      </li>
